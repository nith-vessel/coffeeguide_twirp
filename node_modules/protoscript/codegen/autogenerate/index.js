...
const toMapMessage = (name) => `Object.entries(${name}).map(([key, value]) => ({ key: key ${printIfTypescript("as any")}, value: value ${printIfTypescript("as any")} }))`;
...
function writeProtobufSerializers(types, isTopLevel) {
    ...
    types.forEach((node) => {
        ...
        switch (node.type) {
          ...
          `;
                    if (isEmpty) {
                        result += `decode: function(_bytes${printIfTypescript(`?: ByteSource`)})${printIfTypescript(`: ${node.content.namespacedName}`)} {
              return {};`;
                    }
                    else {
                        result += `decode: function(bytes${printIfTypescript(`: ByteSource`)})${printIfTypescript(`: ${node.content.namespacedName}`)} {
            return ${node.content.namespacedName}._readMessage(${node.content.namespacedName}.initialize(), new BinaryReader(bytes));`;
                    }
                    result += "},\n\n";
                    // initialize
                    result += `\
          /**
           * Initializes ${node.content.namespacedName} with all fields set to their default value.
           */
          initialize: function()${printIfTypescript(`: ${node.content.namespacedName}`)} {
            return {
              ${node.content.fields
                        .map((field) => {
                        if (field.optional) {
        ...
        _writeMessage: function(${printIf(isEmpty, "_")}msg${printIfTypescript(`: ${`Partial<${node.content.namespacedName}>`}`)}, writer${printIfTypescript(`: BinaryWriter`)})${printIfTypescript(`: BinaryWriter`)} {
          ${node.content.fields
                  ...
                  ${field.map
                            ...
                            : `msg.${field.name}`} ${field.write === "writeRepeatedMessage"
                        ...
                        if (field.tsType === "bigint") {
                            if (field.repeated) {
                                res += `msg.${field.name}.map(x => x.toString() ${printIfTypescript("as any")})`;
                            }
                            else {
                                res += `msg.${field.name}.toString() ${printIfTypescript("as any")}`;
                            }
                        }
                        else if (field.read === "readEnum") {
                            if (field.repeated) {
        ...
        `;
                if (isEmpty) {
                    result += `_readMessage: function(_msg${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}, _reader${printIfTypescript(`: BinaryReader`)})${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}{
            return _msg;`;
                }
                else {
                    result += `_readMessage: function(msg${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}, reader${printIfTypescript(`: BinaryReader`)})${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}{
              ...
              switch (field) {
                ${node.content.fields
                        ...
                        if (field.read === "readMessage") {
                            if (field.map) {
                                res += `
                        const map = {}${printIfTypescript(` as ${field.tsType}`)};
                        reader.readMessage(map, ${field.tsType}._readMessage);
                        msg.${field.name}[map.key${printIf(field.tsType !== "string", ".toString()")}] = map.value;
                      `;
                            }
         ...
         */
        _fromInt: `;
                result += `function(i${printIfTypescript(": number")})${printIfTypescript(`: ${node.content.namespacedName}`)} {
          switch (i) {
        `;
        ...
        default: { return i${printIfTypescript(` as unknown as ${node.content.namespacedName}`)}; }\n }\n },\n`;
                // from enum
                result += `\
        /**
         * @private
         */
        _toInt: `;
                result += `function(i${printIfTypescript(`: ${node.content.namespacedName}`)})${printIfTypescript(`: number`)} {
          switch (i) {
        `;
                node.content.values.forEach(({ name, value }) => {
                ...
                result += `// unknown values are preserved as numbers. this occurs when new enum values are introduced and the generated code is out of date.
        default: { return i${printIfTypescript(` as unknown as number`)}; }\n }\n },\n`;
...
function writeJSONSerializers(types, isTopLevel) {
    ...
    types.forEach((node) => {
      ...
      `;
                    if (isEmpty) {
                        result += `decode: function(_json${printIfTypescript(`?: string`)})${printIfTypescript(`: ${node.content.namespacedName}`)} {
          return {};`;
                    }
                    else {
                        result += `decode: function(json${printIfTypescript(`: string`)})${printIfTypescript(`: ${node.content.namespacedName}`)} {
        return ${node.content.namespacedNameJSON}._readMessage(${node.content.namespacedNameJSON}.initialize(), JSON.parse(json));`;
                    }
                    result += "},\n\n";
                    // initialize
                    result += `\
          ...
          ${node.content.fields
                        ...
                        if (field.read === "readMessage") {
                            if (field.repeated) {
                                ...
                                if (field.map) {
                                    res += `const ${name} = ${fromMapMessage(`${toMapMessage(`msg.${field.name}`)}.map(${field.tsTypeJSON}._writeMessage)`)};`;
                                }
                                else {
         ...
         * @private
         */
        _readMessage: function(msg${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}, ${printIf(isEmpty, "_")}json${printIfTypescript(`: any`)})${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}{
          ${node.content.fields
                    .map((field) => {
                    let res = "";
                    const name = `_${field.name}_`;
                    ...
                    if (field.read === "readMessage") {
                        if (field.map) {
                            res += `msg.${field.name} = ${fromMapMessage(`${toMapMessage(name)}.map(${field.tsTypeJSON}._readMessage)`)};`;
                        }
                        else if (field.repeated) {
...
export function printComments(comment) {
...
export function printIfTypescript(str) {
    return printIf(config.isTS, str);
}
function printIf(cond, str) {
    return cond ? str : "";
}
export function generate(fileDescriptorProto, identifierTable, options, plugins) {
    config = {
    ...
    const hasSerializer = !config.typescript.emitDeclarationOnly &&
...
${printIf(!!typeDefinitions, `${printIfTypescript(printHeading("Types"))}
${typeDefinitions}`)}
${printIf(pluginServices.length > 0, pluginServices.join("\n"))}
${printIf(!!protobufSerializers, `${printHeading("Protobuf Encode / Decode")}
${protobufSerializers}
