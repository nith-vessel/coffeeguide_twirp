...
var nodeHttpTransport = (url, options) => {
  const request3 = url.startsWith("https") ? https.request : http.request;
  return new Promise((resolve, reject) => {
    ...
    req.end(
      options.body instanceof Uint8Array ? Buffer.from(options.body) : options.body
    );
  });
};
...
async function runMiddleware(config, request3) {
  let cfg = config;
  let idx = 1;
  const middleware = [...clientMiddleware, request3];
  try {
...
function withRequestLogging(app) {
  app.on("requestReceived", (ctx, request3) => {
    ctx[timingField] = Date.now();
    console.info(`[TwirpScript] Started ${request3.method} "${request3.url}"`);
  });
...
function createTwirpServer(services, config = {}) {
  ...
  async function app(req, res) {
    ...
    const request3 = req;
    request3.body = body;
    const response = await _app(request3);
