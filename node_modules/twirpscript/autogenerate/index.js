...
import { printComments, printHeading, printIfTypescript, } from "protoscript/plugin";
...
function writeClients({ services, packageName }) {
    let result = "";
    services.forEach((service) => {
        ...
        service.methods.forEach((method) => {
            ...
            const path = `/${packageName ? packageName + "." : ""}${service.name}/${method.name}`;
            result += `\
export async function ${method.name}(${input}${printIfTypescript(`: ${method.input}`)}, config${printIfTypescript(`?: ClientConfiguration`)})${printIfTypescript(`: Promise<${method.output}>`)} {
...
`;
    ...
    services.forEach((service) => {
        ...
        service.methods.forEach((method) => {
            ...
            const path = `/${packageName ? packageName + "." : ""}${service.name}/${method.name}`;
            result += `\
export async function ${method.name}JSON(${input}${printIfTypescript(`: ${method.input}`)}, config${printIfTypescript(`?: ClientConfiguration`)})${printIfTypescript(`: Promise<${method.output}>`)} {
  const response = await JSONrequest('${path}', ${method.inputJSON}.encode(${input}), config);
...
function writeServers({ ast: { services, packageName }, config, }) {
    let result = "";
    services.forEach((service) => {
        // print service types
        if (config.isTS) {
        ...
        result += "\n";
        result += `export function create${service.name}${printIfTypescript("<Context>")}(service${printIfTypescript(`: ${service.name}<Context>`)}) { return {
...
export const plugin = ({ ast, config }) => {
    const hasServices = !config.typescript.emitDeclarationOnly && ast.services.length > 0;
    if (!hasServices) {
        return;
    }
    return {
...
${printIfTypescript('import type { ClientConfiguration } from "twirpscript";')}`,
